...
def generateTS(G, length = 10000):
    timestamps = []
    edges = G.edges()
    idx = np.random.choice(G.number_of_edges(), length)
    for tst in range(len(idx)):
        idx_tst = int(idx[tst])
        edges_list = list(edges)
        n1, n2 = edges_list[idx_tst]
        if n2 < n1:
            n1, n2 = n2, n1            
        timestamps.append((tst, n1, n2))
    return timestamps
...
def generateTS_planted(G, length = 100, k = 10, clique_size = 5, interval = 10):
    timestamps = []
    edges = G.edges()
    idx = np.random.choice(G.number_of_nodes(), length)
    for tst in range(length):        
        n1, n2 = edges[idx[tst]]
        if n2 < n1:
            n1, n2 = n2, n1            
